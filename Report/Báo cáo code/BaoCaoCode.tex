...
\usepackage{titlesec}
\titlespacing*{\section}{0pt}{24pt}{20pt}
\titleformat{\section}{\fontsize{14pt}{0pt}\selectfont \bfseries \centering}{}{0pt}{}
\titlespacing*{\subsection}{0pt}{16pt}{10pt}
\titleformat*{\subsection}{\fontsize{13pt}{0pt}\selectfont \bfseries}
\titlespacing*{\subsubsection}{8pt}{12pt}{6pt}
...
\usepackage[unicode,pdfencoding=auto,colorlinks=true,linkcolor=blue,urlcolor=blue,citecolor=blue,pdfborder=0 0 0]{hyperref}
...
%==============================================================================
\section{Chương 1: Tổng quan cấu trúc và các module firmware}
%==============================================================================

\subsection{Tổng quan cấu trúc mã nguồn}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{DataManager/} -- \texttt{Common.c}, \texttt{Common.h}: Định nghĩa các kiểu dữ liệu dùng chung (\texttt{DataManager\_t}, \texttt{menu\_item\_t}, \texttt{menu\_list\_t}, \texttt{objectInfoManager\_t}, \texttt{SelectionParam\_t}, \ldots). (Các kiểu khác xem \texttt{Common.h}.)
    ...
    \begin{itemize}
        \item \textbf{SensorTypes/} -- \texttt{SensorTypes.h}: Định nghĩa \texttt{SensorType\_t}, \texttt{PortId\_t}, \\
        \texttt{TypeCommunication\_t}, 
        \texttt{SensorData\_t}, \texttt{sensor\_driver\_t}. (Chi tiết ở mục 1.6.1.)
    ...
    \begin{itemize}
        \item \textbf{MenuSystem/} -- \texttt{MenuSystem.c}, \texttt{MenuSystem.h}: Cây menu Root $\to$ WiFi Config, Sensors, Actuators, Battery Status, Information; Sensors $\to$ Port 1/2/3 $\to$ UART/I2C/SPI/ANALOG/PULSE $\to$ danh sách cảm biến; task \texttt{MenuNavigation\_Task}.
...
\begin{enumerate}
    ...
    \item \textbf{Dữ liệu Port:} Gán \texttt{DataManager.selectedSensor[i] = SENSOR\_NONE} cho mọi port \texttt{i} trước khi gọi \texttt{MenuSystemInit}, để menu Sensors lúc khởi động hiển thị ``Port 1'', ``Port 2'', ``Port 3'' (tránh zero-init hiển thị nhầm ``Port 1 - BME280'').
...
\subsection{DataManager (Common) -- Dữ liệu dùng chung}
...
\begin{itemize}
    ...
    \item \textbf{Thông tin đối tượng:} \texttt{objectInfoManager\_t} (batteryInfo, wifiInfo, \\ \texttt{selectedSensorName[NUM\_PORTS]}). \\
    \texttt{selectedSensorName} do MenuSystem cập nhật từ \texttt{DataManager->selectedSensor}, dùng trong callback và màn hình dữ liệu cảm biến.
    \item \textbf{DataManager\_t:} \texttt{sensor}, \texttt{button}, \texttt{screen}, \texttt{objectInfo}, \texttt{MenuReturn[10]}, \\ \texttt{selectedSensor[NUM\_PORTS]}; cùng hai callback: \texttt{on\_sensor\_selected(data, port)} (cập nhật tên menu, quay về menu Sensors), \texttt{on\_ports\_reset(data)} (đưa tên Port về ``Port 1'', ``Port 2'', ``Port 3'').
...
\subsection{SensorTypes và SensorRegistry}

\subsubsection{SensorTypes.h}

Định nghĩa:
\begin{itemize}
    \item \texttt{PortId\_t} (PORT\_1, PORT\_2, PORT\_3), \texttt{NUM\_PORTS = 3}, \texttt{SensorType\_t} (SENSOR\_BME280, SENSOR\_MHZ14A, \ldots).
    \item \texttt{TypeCommunication\_t}: COMMUNICATION\_UART, I2C, SPI, ANALOG, PULSE (nhóm menu theo giao tiếp).
...
\begin{itemize}
    ...
    \item \textbf{API:} \texttt{sensor\_registry\_get\_drivers()}, \texttt{sensor\_registry\_get\_count()}, \\ \texttt{sensor\_registry\_get\_driver(sensor\_type)}, \\
    \texttt{sensor\_registry\_get\_count\_by\_interface(iface)}, \\ \texttt{sensor\_registry\_get\_driver\_at\_interface(iface, index, out\_sensor\_type)}. Hai hàm cuối dùng để xây menu theo giao tiếp (UART, I2C, SPI, ANALOG, PULSE).
\end{itemize}

\subsubsection{Sơ đồ cây cảm biến theo giao tiếp}
...
\subsection{MenuSystem -- Cấu trúc menu và điều hướng}

\subsubsection{Cây menu}

\begin{itemize}
    \item \textbf{Root:} WiFi Config, Sensors, Actuators, Battery Status, Information.
    \item \textbf{Sensors:} Port 1, Port 2, Port 3, Show data sensor, Reset All Ports. Mỗi Port 1/2/3 mở ra menu UART, I2C, SPI, ANALOG, PULSE; mỗi giao tiếp mở ra danh sách cảm biến tương ứng (driver có \texttt{interface} trùng). Chọn cảm biến $\to$ gọi \texttt{select\_sensor\_cb}: chỉ khi driver init thành công mới gán \texttt{Data->selectedSensor[port]}; nếu thất bại thì \texttt{selectedSensor[port] = SENSOR\_NONE}, menu hiển thị ``Port X''. Callback \texttt{on\_sensor\_selected} (MenuSystem) cập nhật \texttt{Sensor\_Menu\_Items[0..2].name} và quay màn hình về menu Sensors, highlight port vừa chọn.
    ...
    \item \textbf{WiFi Config / Battery Status:} Callback do FunctionManager đảm nhiệm.
    \item \textbf{Information:} \texttt{information\_callback} (FunctionManager) gọi \texttt{ScreenShowInformation} hiển thị thông tin project (MRS Project, module quản lý cảm biến đa năng, tác giả MrKoi); sau vài giây tự vẽ lại Root menu.
\end{itemize}

\subsubsection{Sơ đồ cây menu (Menu Tree)}
...
\subsubsection{Khởi tạo menu động theo giao tiếp}

Hàm \texttt{init\_sensor\_interface\_port\_menu\_items(Data)}:
\begin{itemize}
    \item Với mỗi cặp (port, interface): lấy \\ \texttt{count = sensor\_registry\_get\_count\_by\_interface(iface)}, cấp phát \\ \texttt{SelectionParam\_t} và \texttt{menu\_item\_t} cho từng cảm biến, điền \\ \texttt{PortInterfaceMenus[port][i]}, \texttt{PortMenus[port].port\_index = port}.
...
\subsubsection{Cập nhật tên Port và quay về menu Sensors}

MenuSystem dùng hàm nội bộ \texttt{update\_port\_names(Data)}: dựa trên \\ \texttt{data->selectedSensor[0..2]} gán \texttt{Sensor\_Menu\_Items[i].name} là ``Port $i+1$'' nếu \texttt{selectedSensor[i] == SENSOR\_NONE}, hoặc ``Port $i+1$ - tên cảm biến'' nếu đã chọn và init thành công (FunctionManager chỉ gán \texttt{selectedSensor[port]} trong trường hợp đó).

Hàm \texttt{go\_to\_sensor\_menu(Data, selected\_index)} đặt \texttt{data->screen.current = \&Sensor\_Menu}, \texttt{data->screen.selected = selected\_index} và gọi \texttt{MenuRender}. Trong \texttt{MenuSystemInit}, DataManager được gán \texttt{on\_sensor\_selected} và \texttt{on\_ports\_reset}; \texttt{Root\_Items[4].ctx = Data} để \texttt{information\_callback} nhận DataManager. Khi người dùng chọn xong cảm biến, FunctionManager gọi \texttt{on\_sensor\_selected} $\to$ MenuSystem cập nhật tên và quay về menu Sensors; khi chọn ``Reset All Ports'', FunctionManager gọi \texttt{on\_ports\_reset} $\to$ MenuSystem đưa tên ba mục Port về ``Port 1'', ``Port 2'', ``Port 3''.

\subsubsection{Task điều hướng}

\texttt{MenuNavigation\_Task} lặp vô hạn: đọc \texttt{ReadButtonStatus()}; nếu UP/DOWN thì thay đổi \texttt{selected} và gọi \texttt{MenuRender}; nếu SEL thì gọi callback của mục đang chọn hoặc chuyển vào \texttt{children}; nếu BACK thì chuyển về \texttt{parent}.

Trước mỗi lần xử lý, cập nhật \texttt{data->objectInfo.selectedSensorName[p] = sensor\_type\_to\_name(data->selectedSensor[p])} cho mọi port \texttt{p} (phục vụ callback và màn hình dữ liệu cảm biến).

\subsection{ScreenManager -- Hiển thị OLED}
...
\begin{itemize}
    ...
    \item \texttt{ScreenShowInformation(lines, n\_lines)}: Hiển thị nhiều dòng text lên OLED (mỗi dòng cách nhau 12\,px), dùng cho màn \textbf{Information} (thông tin project, tác giả MrKoi). Dùng chung mutex và \texttt{ssd1306\_refresh\_gram}.
    \item \texttt{ScreenWifiConnecting}, \texttt{ScreenShowMessage}, \texttt{ScreenShowDataSensor}: Các màn hình đặc biệt khác, đều dùng chung mutex và \texttt{ssd1306\_refresh\_gram}.
\end{itemize}

\subsubsection{Mutex và timeout I2C}
...
\subsection{FunctionManager -- Callback menu}
...
\begin{itemize}
    ...
    \item \textbf{select\_sensor\_cb:} Nhận \texttt{SelectionParam\_t} (data, port, sensor). Chỉ gán \\ \texttt{data->selectedSensor[port] = sensor} khi driver init thành công (hoặc port đã chọn/sensor đã init); nếu không có init hoặc init thất bại thì gán \texttt{selectedSensor[port] = SENSOR\_NONE} để menu hiển thị ``Port X''. Sau đó gọi \texttt{data->on\_sensor\_selected(data, port)} (MenuSystem) để cập nhật tên \texttt{Sensor\_Menu\_Items[0..2]} và quay màn hình về menu Sensors, highlight port vừa chọn.
    \item \textbf{show\_data\_sensor\_cb:} Bật flag hiển thị màn hình dữ liệu cảm biến cho port tương ứng.
    \item \textbf{reset\_all\_ports\_callback:} Đặt \texttt{selectedSensor[i] = SENSOR\_NONE} cho mọi port \texttt{i}; gọi \texttt{data->on\_ports\_reset(data)} để MenuSystem đưa \\ \texttt{Sensor\_Menu\_Items[0..2].name} về ``Port 1'', ``Port 2'', ``Port 3''; dọn task đọc cảm biến.
    ...
    \item \textbf{information\_callback:} Nhận \texttt{ctx} = DataManager. Gọi \texttt{ScreenShowInformation} với các dòng (MRS Project, module quản lý cảm biến đa năng, tác giả MrKoi); delay 4\,s; \texttt{MenuRender} lại Root menu.
...
\subsection{Cấu hình pin và Kconfig}

\subsubsection{main/Kconfig.projbuild}
...
\subsubsection{component/drivers/i2cdev/Kconfig.projbuild}
...
\subsubsection{Cấu hình và tác dụng của chân trên từng Port}

Hình~\ref{fig:port-config} mô tả sơ đồ chân (pinout) của Port 1, Port 2 và Port 3: mỗi port có 10 chân với nguồn 5V, 3V3, GND; Port 1 gồm MISO, SCK, MOSI, CS (SPI) và Analog/Pulse/GPIO; Port 2 gồm Pulse/GPIO, Analog/GPIO, SDA/SCL (I2C); Port 3 gồm TX2/RX2 (UART), Analog/Pulse/GPIO, SDA/SCL (I2C). Các chân được khai báo trong menuconfig và dùng trong firmware qua \texttt{CONFIG\_*} tương ứng.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Image/PortConfig.png}
\caption{Cấu hình và tác dụng chân trên Port 1, Port 2, Port 3 (5V, 3V3, GND, SPI/I2C/UART/Analog/Pulse/GPIO).}
\label{fig:port-config}
\end{figure}

\textbf{Giải thích chân đa năng:}
\begin{itemize}
    \item \textbf{Analog/Pulse/GPIO:} Chân này có thể dùng để đọc input Analog, GPIO và output: PWM, GPIO.
    \item \textbf{Pulse/GPIO:} Chỉ là output và input GPIO, không có Analog. Hai chân Pulse/GPIO (trên sơ đồ) dùng cho giao tiếp Pulse 2 chân (DHT11, HC-SR04, \ldots).
\end{itemize}

%==============================================================================
\section{Chương 2: Triển khai build, chạy và hướng dẫn mở rộng}
%==============================================================================

\textbf{Lưu ý (menuconfig):} Các chân GPIO (UART, SPI, I2C, IO port) và tùy chọn build được \textbf{khai báo trong menuconfig}: chạy \texttt{idf.py menuconfig}, vào menu ``Pin config'' (main) và ``I2C common config'' (driver i2cdev) để đặt SDA/SCL, TX/RX, IO\_1\_PORT\_1, IO\_3\_PORT\_2, \ldots theo đúng phần cứng và target (ESP32/ESP32-C6). Giá trị này được dùng trong \texttt{main.c} và các driver qua \texttt{CONFIG\_*}.
...
\begin{enumerate}
    \item Người dùng mở Sensors $\to$ Port 1 $\to$ I2C $\to$ chọn BME280. \texttt{select\_sensor\_cb} gọi \texttt{driver->init()}; chỉ khi init thành công mới gán \texttt{selectedSensor[PORT\_1] = SENSOR\_BME280}, rồi gọi \texttt{on\_sensor\_selected} $\to$ màn hình quay về menu Sensors, mục đầu tiên hiển thị ``Port 1 - BME280''. Nếu init thất bại hoặc không có init, \texttt{selectedSensor[PORT\_1]} giữ \texttt{SENSOR\_NONE} nên menu vẫn hiển thị ``Port 1''.
    ...
    \item Khi chọn ``Reset All Ports'', \texttt{on\_ports\_reset} được gọi $\to$ tên ba mục Port trong menu Sensors trở lại ``Port 1'', ``Port 2'', ``Port 3''.
    \item Người dùng mở Actuators $\to$ IO1-Port1 $\to$ ON: \texttt{actuator\_on\_cb} đặt GPIO \\ \texttt{CONFIG\_IO\_1\_PORT\_1} lên 1.
\end{enumerate}

\subsection{Hướng dẫn chi tiết cách thêm cảm biến mới}

Để thêm một loại cảm biến mới vào firmware MSMS, làm lần lượt các bước dưới đây. \textbf{Thứ tự quan trọng:} triển khai \texttt{init}/\texttt{read}/\texttt{deinit} trước; sau đó mới đăng ký driver trong \texttt{SensorRegistry.c} (gán con trỏ hàm vào \texttt{sensor\_drivers[]}). Sau khi hoàn tất, cảm biến sẽ tự xuất hiện trong menu Sensors $\to$ Port 1/2/3, nhánh UART / I2C / SPI / ANALOG / PULSE, tương ứng với \texttt{interface} đã gán (không cần sửa MenuSystem).

\subsubsection{Bước 1: Thêm định danh cảm biến trong \texttt{SensorTypes.h}}

Thêm một phần tử mới vào enum \texttt{SensorType\_t}, đặt ngay trước \texttt{SENSOR\_NONE} nếu có, hoặc theo thứ tự mong muốn (thứ tự enum phải trùng với thứ tự phần tử trong mảng \texttt{sensor\_drivers[]} ở Bước 3).
...
\subsubsection{Bước 2: Triển khai init/read/deinit}

Triển khai trước các hàm \texttt{init}, \texttt{read}, \texttt{deinit} của cảm biến. Nếu cảm biến dùng driver riêng (ví dụ component \texttt{component/drivers/XYZ/}), triển khai \texttt{system\_err\_t xyzInitialize(void)}, \texttt{xyzReadData(SensorData\_t *)}, \texttt{xyzDeinitialize(void)}. Nếu logic đọc/init nằm trong \texttt{SensorConfig} (như BME280), có thể gọi từ \texttt{SensorConfig}; khi đăng ký vào registry sẽ gán con trỏ tới các hàm đó. Đảm bảo \texttt{read} ghi dữ liệu vào \texttt{data->data\_fl[]} hoặc \texttt{data->data\_uint32[]} theo \texttt{unit\_count} và \texttt{description} sẽ khai báo ở bước đăng ký.

\subsubsection{Bước 3: Đăng ký driver trong \texttt{SensorRegistry.c}}

Sau khi đã có \texttt{init}/\texttt{read}/\texttt{deinit}, \\
thêm một phần tử \texttt{sensor\_driver\_t} vào mảng \texttt{sensor\_drivers[]} (trong \texttt{SensorRegistry.c}).

Các trường bắt buộc: \texttt{name}, \texttt{description}, \texttt{unit}, \texttt{unit\_count}, \texttt{interface} (UART, I2C, SPI, ANALOG, PULSE), và các con trỏ hàm \texttt{init}, \texttt{read}, \texttt{deinit} trỏ tới các hàm đã triển khai ở Bước 2.
...
\subsubsection{Bước 4: Cập nhật \texttt{sensor\_type\_to\_name()} trong \texttt{SensorRegistry.c}}
...
\subsubsection{Bước 5: (Tùy chọn) Cấu hình Kconfig và GPIO}
...
\subsubsection{Tóm tắt quy trình thêm cảm biến}
...
\begin{enumerate}
    ...
    \item Đăng ký driver: thêm phần tử \texttt{sensor\_driver\_t} vào \texttt{sensor\_drivers[]} trong \texttt{SensorRegistry.c}, gán \texttt{interface} và con trỏ \texttt{init}/\texttt{read}/\texttt{deinit}.
