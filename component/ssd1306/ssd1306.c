...
void ssd1306_draw_num(ssd1306_handle_t dev, uint8_t chXpos, uint8_t chYpos,
  ...
  for (i = 0; i < chLen; i++) {
    ...
    if (chShow == 0 && i < (chLen - 1)) {
      if (chTemp == 0) {
        ssd1306_draw_char(dev, chXpos + (chSize / 2) * i, chYpos, ' ', chSize,
                          1);
        continue;
      } else {
        chShow = 1;
    ...
    ssd1306_draw_char(dev, chXpos + (chSize / 2) * i, chYpos, chTemp + '0',
                      chSize, 1);
  }
}
...
void ssd1306_clear_region(ssd1306_handle_t dev, int x, int y, int width,
                          int height) {
  int x1 = x;
  int y1 = y;
  int x2 = x + width - 1;
  ...
  if (x2 >= SSD1306_WIDTH)
    x2 = SSD1306_WIDTH - 1;
  if (y2 >= SSD1306_HEIGHT)
    y2 = SSD1306_HEIGHT - 1;
  if (x1 > x2 || y1 > y2)
    return;

  ssd1306_fill_rectangle(dev, (uint8_t)x1, (uint8_t)y1, (uint8_t)x2,
                         (uint8_t)y2, 0);
}